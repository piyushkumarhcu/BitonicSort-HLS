...
for(dloop_t i=0; i<N/4; i++){
    #pragma HLS unroll
    result = AscendDescend(in1[4*i], in1[4*i+1]);
    a[4*i] = result.smaller; a[4*i+1] = result.greater; 
}

for(dloop_t i=0; i<N/4; i++){
    #pragma HLS unroll
    result = AscendDescend(in1[4*i+2], in1[4*i+3]);
...
for(dloop_t i=0; i<N/8; i++){
    #pragma HLS unroll
    FourinSmallFirInc(a[8*i], a[8*i+1], a[8*i+2], a[8*i+3], b[8*i], b[8*i+1], b[8*i+2], b[8*i+3]);
    FourinGreatFirInc(a[8*i+4], a[8*i+5], a[8*i+6], a[8*i+7], b[8*i+4], b[8*i+5], b[8*i+6], b[8*i+7]); 
}
...
for(dloop_t i=0; i<N/16; i++){
    #pragma HLS unroll
EightinSmallFirInc(c[16*i+0], c[16*i+1], c[16*i+2], c[16*i+3], c[16*i+4], c[16*i+5], c[16*i+6], c[16*i+7],
                 d[16*i+0], d[16*i+1], d[16*i+2], d[16*i+3], d[16*i+4], d[16*i+5], d[16*i+6], d[16*i+7]);
EightinGreatFirInc(c[16*i+8], c[16*i+9], c[16*i+10], c[16*i+11], c[16*i+12], c[16*i+13], c[16*i+14], c[16*i+15],
...
for(dloop_t i=0; i<N/16; i++){
    #pragma HLS unroll
        FourinSmallFirInc(d[16*i], d[16*i+1], d[16*i+2], d[16*i+3], e[16*i], e[16*i+1], e[16*i+2], e[16*i+3]);
        FourinSmallFirInc(d[16*i+4], d[16*i+5], d[16*i+6], d[16*i+7], e[16*i+4], e[16*i+5], e[16*i+6], e[16*i+7]);
        FourinGreatFirInc(d[16*i+8], d[16*i+9], d[16*i+10], d[16*i+11], e[16*i+8], e[16*i+9], e[16*i+10], e[16*i+11]);
        FourinGreatFirInc(d[16*i+12], d[16*i+13], d[16*i+14], d[16*i+15], e[16*i+12], e[16*i+13], e[16*i+14], e[16*i+15]);
}
...
SixteenSmallFirInc(f[0], f[1], f[2], f[3], f[4], f[5], f[6], f[7],
                 f[8], f[9], f[10], f[11], f[12], f[13], f[14], f[15], 
                 g[0], g[1], g[2], g[3], g[4], g[5], g[6], g[7], g[8], 
                 g[9], g[10], g[11], g[12], g[13], g[14], g[15]);
SixteenGreatFirInc(f[16], f[17], f[18], f[19], f[20], f[21], f[22], f[23],
...
for(dloop_t i=0; i<N/16; i++){
    #pragma HLS unroll
    EightinSmallFirInc(g[8*i], g[8*i+1], g[8*i+2], g[8*i+3], g[8*i+4], g[8*i+5], g[8*i+6], g[8*i+7],
                 h[8*i], h[8*i+1], h[8*i+2], h[8*i+3], h[8*i+4], h[8*i+5], h[8*i+6], h[8*i+7]);
}

for(dloop_t i=0; i<N/16; i++){
    #pragma HLS unroll
    EightinGreatFirInc(g[8*i+16], g[8*i+17], g[8*i+18], g[8*i+19], g[8*i+20], g[8*i+21], g[8*i+22], g[8*i+23],
...
for(dloop_t i=0; i<N/8; i++){
    #pragma HLS unroll
    FourinSmallFirInc(h[4*i], h[4*i+1], h[4*i+2], h[4*i+3], l[4*i], l[4*i+1], l[4*i+2], l[4*i+3]);
}

for(dloop_t i=0; i<N/8; i++){
    #pragma HLS unroll
    FourinGreatFirInc(h[4*i+16], h[4*i+17], h[4*i+18], h[4*i+19], l[4*i+16], l[4*i+17], l[4*i+18], l[4*i+19]);
}
...
for(dloop_t i=0; i<N/16; i++){
    #pragma HLS unroll
    SixteenSmallFirInc(n[16*i], n[16*i+1], n[16*i+2], n[16*i+3], n[16*i+4], n[16*i+5], n[16*i+6], n[16*i+7],
...
for(dloop_t i=0; i<N/8; i++){
    #pragma HLS unroll
    EightinSmallFirInc(o[8*i], o[8*i+1], o[8*i+2], o[8*i+3], o[8*i+4], o[8*i+5], o[8*i+6], o[8*i+7],
...
for(dloop_t i=0; i<N/4; i++){
    #pragma HLS unroll
    FourinSmallFirInc(p[4*i], p[4*i+1], p[4*i+2], p[4*i+3], q[4*i], q[4*i+1], q[4*i+2], q[4*i+3]);
}
...
for(dloop_t i=0; i<N/2; i++){
    ...
    out1[2*i] = result.smaller; out1[2*i+1] = result.greater;
}
