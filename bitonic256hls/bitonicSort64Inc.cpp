...
void bitonicSort64Inc(din_t in3[M], din_t out3[M]){
    ...
    #pragma HLS ARRAY_PARTITION variable=in3
    #pragma HLS ARRAY_PARTITION variable=out3
...
for(dloop_t i=0; i<M/32; i++){
    #pragma HLS UNROLL
    ThirtyTwoInSmallFirInc(s[32*i], s[32*i+1], s[32*i+2], s[32*i+3], s[32*i+4], s[32*i+5], s[32*i+6], s[32*i+7],
...
for(dloop_t i=0; i<M/16; i++){
    #pragma HLS UNROLL
    SixteenSmallFirInc(t[16*i], t[16*i+1], t[16*i+2], t[16*i+3], t[16*i+4], t[16*i+5], t[16*i+6], t[16*i+7],
...
for(dloop_t i=0; i<M/4; i++){
    #pragma HLS UNROLL
    FourinSmallFirInc(v[4*i], v[4*i+1], v[4*i+2], v[4*i+3], w[4*i], w[4*i+1], w[4*i+2], w[4*i+3]);
}
...
for(dloop_t i=0; i<M/2; i++){
    ...
    out3[2*i] = result.smaller; out3[2*i+1] = result.greater;
}
