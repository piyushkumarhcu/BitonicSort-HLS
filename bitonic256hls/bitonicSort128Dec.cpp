...
void bitonicSort128Dec(din_t in6[O], din_t out6[O]){
    ...
    #pragma HLS ARRAY_PARTITION variable=in6
    #pragma HLS ARRAY_PARTITION variable=out6
...
for(dloop_t i=0; i<O/2; i++){
    ...
    s[i] = result.greater; s[i+O/2] = result.smaller;
}
...
for(dloop_t i=0; i<O/64; i++){
    #pragma HLS UNROLL
   SixtyFourInSmallFirDec(s[64*i+0], s[64*i+1], s[64*i+2], s[64*i+3], s[64*i+4], s[64*i+5], s[64*i+6], s[64*i+7],
...
for(dloop_t i=0; i<O/32; i++){
    #pragma HLS UNROLL
    ThirtyTwoInSmallFirDec(t[32*i], t[32*i+1], t[32*i+2], t[32*i+3], t[32*i+4], t[32*i+5], t[32*i+6], t[32*i+7],
...
for(dloop_t i=0; i<O/16; i++){
    #pragma HLS UNROLL
    SixteenSmallFirDec(u[16*i+0], u[16*i+1], u[16*i+2], u[16*i+3], u[16*i+4], u[16*i+5], u[16*i+6], u[16*i+7], 
...
for(dloop_t i=0; i<O/8; i++){
    #pragma HLS UNROLL
    EightinSmallFirDec(v[8*i+0], v[8*i+1], v[8*i+2], v[8*i+3], v[8*i+4], v[8*i+5], v[8*i+6], v[8*i+7],
...
for(dloop_t i=0; i<O/4; i++){
    #pragma HLS UNROLL
    FourinSmallFirDec(w[4*i+0], w[4*i+1], w[4*i+2], w[4*i+3], x[4*i+0], x[4*i+1], x[4*i+2], x[4*i+3]);
}
...
for(dloop_t i=0; i<O/2; i++){
    ...
    out6[2*i] = result.greater; out6[2*i+1] = result.smaller;
}
