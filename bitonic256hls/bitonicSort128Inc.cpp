...
void bitonicSort128Inc(din_t in5[O], din_t out5[O]){
    ...
    #pragma HLS ARRAY_PARTITION variable=in5
    #pragma HLS ARRAY_PARTITION variable=out5
...
for(dloop_t i=0; i<O/64; i++){
    #pragma HLS UNROLL
   SixtyFourInSmallFirInc(s[64*i+0], s[64*i+1], s[64*i+2], s[64*i+3], s[64*i+4], s[64*i+5], s[64*i+6], s[64*i+7],
...
for(dloop_t i=0; i<O/2; i++){
    ...
    out5[2*i] = result.smaller; out5[2*i+1] = result.greater;
}
